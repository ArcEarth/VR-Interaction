...
namespace OVR {
	namespace Render {
		void Model::SetHightlight(Color color)
		{
		...
		{
			return (Point - Center) < Extents;
		}

		bool BoundingBox::Intersects(const Ray & ray, float* dist)
		{
			auto& Origin = ray.Origin;
			auto& Direction = ray.Direction;

			float epsilon = 1e-6;
			auto Epsilon = Vector3f(epsilon, epsilon, epsilon);

			// Adjust ray origin to be relative to center of the box.
			Vector3f TOrigin = Center - Origin;

			//// Compute the dot product againt each axis of the box.
			//// Since the axii are (1,0,0), (0,1,0), (0,0,1) no computation is necessary.
			Vector3f AxisDotOrigin = TOrigin;
			Vector3f AxisDotDirection = Direction;

			//// if (fabs(AxisDotDirection) <= Epsilon) the ray is nearly parallel to the slab.
			auto IsParallel = AxisDotDirection < Epsilon;

			//// Test against all three axii simultaneously.
			//Vector3f InverseAxisDotDirection = XMVectorReciprocal(AxisDotDirection);
			//Vector3f t1 = (AxisDotOrigin - Extents) * InverseAxisDotDirection;
			//Vector3f t2 = (AxisDotOrigin + Extents) * InverseAxisDotDirection;

			//// Compute the max of min(t1,t2) and the min of max(t1,t2) ensuring we don't
			//// use the results from any directions parallel to the slab.
			//Vector3f t_min = XMVectorSelect(XMVectorMin(t1, t2), g_FltMin, IsParallel);
			//Vector3f t_max = XMVectorSelect(XMVectorMax(t1, t2), g_FltMax, IsParallel);
			...
			//Vector3f NoIntersection = XMVectorGreater(XMVectorSplatX(t_min), XMVectorSplatX(t_max));

			//// if ( t_max < 0.0f ) return false;
			//NoIntersection = XMVectorOrInt(NoIntersection, XMVectorLess(XMVectorSplatX(t_max), XMVectorZero()));

			//// if (IsParallel && (-Extents > AxisDotOrigin || Extents < AxisDotOrigin)) return false;
			//Vector3f ParallelOverlap = XMVectorInBounds(AxisDotOrigin, Extents);
