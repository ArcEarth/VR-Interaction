...
int get_mouse_gesture(int mousenum);
...
typedef struct STRUCT_RAW_MOUSE {
	  ...
	  long Gesture,GestureBuffer,GestureBufferTime;

	  long buttonpressed[MAX_RAW_MOUSE_BUTTONS],ButtonBuffer[MAX_RAW_MOUSE_BUTTONS],ButtonBufferTime[MAX_RAW_MOUSE_BUTTONS];
...
BOOL init_raw_mouse(BOOL in_include_sys_mouse, BOOL in_include_rdp_mouse, BOOL in_include_individual_mice)
{
	...
	for (i=0;i<nraw_mouse_count;i++){
		...
		{
			raw_mice[i].buttonpressed[j]=0;
			raw_mice[i].ButtonBuffer[j]=-1;
			raw_mice[i].ButtonBufferTime[j]=0;
		}
	}
	bHasBeenInitialized = 1;
	return 1;  
...
/*					if (raw_mice[i].is_absolute) {
					...
					if (raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_1_DOWN) raw_mice[i].ButtonBuffer[0] = 1;
					if (raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_1_UP)	raw_mice[i].ButtonBuffer[0] = 0;
					if (raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_2_DOWN) raw_mice[i].ButtonBuffer[1] = 1;
					if (raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_2_UP)   raw_mice[i].ButtonBuffer[1] = 0;
					if (raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_3_DOWN) raw_mice[i].ButtonBuffer[2] = 1;
					if (raw->data.mouse.usButtonFlags & RI_MOUSE_BUTTON_3_UP)   raw_mice[i].ButtonBuffer[2] = 0;
...
*/
	  ...
	  return raw->data.mouse.usButtonFlags;
}
...
int get_mouse_gesture(int mousenum) {
	// It's ok to ask if buttons are pressed for unitialized mice - just tell 'em no button's pressed
	if (mousenum >= nraw_mouse_count|| raw_mice == NULL) return -1;
	return (raw_mice[mousenum].Gesture);
}
...
void RefreshButtonBuffer(){
	...
	for (i=0;i<nraw_mouse_count;i++){
		...
		for (j = 0; j < 3; j++)
		{
			// && now-raw_mice[i].ButtonBufferTime[j]>=75
			if(raw_mice[i].ButtonBuffer[j]!=-1){
				raw_mice[i].buttonpressed[j]=raw_mice[i].ButtonBuffer[j];
				raw_mice[i].ButtonBuffer[j]=-1;
			}
			gest|=raw_mice[i].buttonpressed[j]<<j;
		}
		raw_mice[i].Gesture=gest;
/*		switch (gest)
		{
		case 0x001:
			raw_mice[i].Gesture=1;
			break;
		case 0x010:
			raw_mice[i].Gesture=2;
			break;
		case 0x100:
			raw_mice[i].Gesture=3;
			break;
		case 0x101:
			raw_mice[i].Gesture=4;
			break;
		case 0x111:
			raw_mice[i].Gesture=4;
			break;
		default:
			raw_mice[i].Gesture=0;
			break;
		}*/
	}
