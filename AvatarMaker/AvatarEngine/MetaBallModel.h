...
namespace Geometrics{
	...
	public:
		...
		Metaball(const DirectX::XMFLOAT3 &_Position,float _Radius,unsigned int _BindingIndex = 0);
		Metaball(const DirectX::XMFLOAT4 & Sphere,unsigned int _BindingIndex = 0);
	...
	public:
		...
		void Tessellate(std::vector<_Tvertex> &Vertices,std::vector<unsigned int> &Indices,float precise);
	...
	template <typename _Tvertex>
	void MetaBallModel::Tessellate(std::vector<_Tvertex> &Vertices,std::vector<unsigned int> &Indices,float precise)
	{
		if (this->size() == 0) 
			return;
		...
		{
			Vertices[i].Position = (XMFLOAT4)polygonizer.get_vertex(i);
