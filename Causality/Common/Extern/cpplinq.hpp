...
namespace cpplinq
{
    ...
    namespace detail
    {
        ...
        struct max_selector_builder : base_builder
        {
            ...
            template<typename TRange>
            CPPLINQ_INLINEMETHOD typename auto build (TRange range) -> decltype(selector(range.front())) const
            {
                auto current = std::numeric_limits<decltype(selector(range.front()))>::min();
                while (range.next ())
                {
        ...
        struct min_selector_builder : base_builder
        {
			...
			CPPLINQ_INLINEMETHOD typename auto build(TRange range) -> decltype(selector(range.front())) const
			{
				auto current = std::numeric_limits<decltype(selector(range.front()))>::max();
				while (range.next ())
                {
                    auto v = selector (range.front ());
                    if (v < current)
