...
namespace OVR {
	namespace RenderTiny {
		...
		inline const btQuaternion& Ovr2Bt(const Quatf &q)
		{
			return reinterpret_cast<const btQuaternion&>(q);
		}
		inline const btVector3& Ovr2Bt(const Vector3f &v)
		{
			return reinterpret_cast<const btVector3&>(v);
		}
		inline const Quatf& Bt2Ovr(const btQuaternion &q)
		{
			return reinterpret_cast<const Quatf&>(q);
		}
		inline const Vector3f& Bt2Ovr(const btVector3 &v)
		{
		...
		public:
			...
			virtual const Matrix4f&  GetMatrix() const = 0;

			virtual void     Render(const Matrix4f& ltw, RenderDevice* ren) { OVR_UNUSED2(ltw, ren); }
		};

		// Static Node , the Position/Orientation is predefined and not bound to Physics Simulation
		...
		public:
			SNode() : Pos(Vector3f(0)), MatCurrent(1) { }
			virtual ~SNode() { }

			virtual const Vector3f&  GetPosition() const      { return Pos; }
			virtual const Quatf&     GetOrientation() const   { return Rot; }
			virtual void             SetPosition(Vector3f p)  { Pos = p; MatCurrent = 0; }
			virtual void             SetOrientation(Quatf q)  { Rot = q; MatCurrent = 0; }
			...
			virtual void  SetMatrix(const Matrix4f& m)
			{
				MatCurrent = true;
				Mat = m;
			}
		...
		//------------------------------------------------------------------------------------
		// PhysicalNode is bound to an Physics Collision model inside to automatically position and orientation itself
		class PNode
			: public Node
		{
		public:
			PNode(btDynamicsWorld *pWorld)
				: m_pWorld(pWorld)
			{ }
			...
			void SetShape(const std::shared_ptr<btCollisionShape>& pShape, float mass, Vector3f Pos, Quatf Rot)
			{
				m_pShape = pShape;
			...
			virtual const Matrix4f&  GetMatrix() const
			{
				...
				//m_pRigidBody->getWorldTransform().getOpenGLMatrix((float*) &m_TransformMatrix);
		...
		public:
			...
			Model(btDynamicsWorld*pWorld , PrimitiveType t = Prim_Triangles)
				: PNode(pWorld), Type(t), Fill(NULL), Visible(true) {}
			~Model() { }
		...
		public:
			...
			{
				pDynamicsWorld->stepSimulation(1.0f / frameRate, 10);
			}
