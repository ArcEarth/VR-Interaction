...
void OculusWorldDemoApp::OnAudioCaptureDeviceStateChanged(void* sender,const DeviceStateChangedEventArgs* e)
{
	...
	case DeviceState::DeviceStateCapturing:
		ShowStatusMessage("Device is Capturing Audio data", NotifyType::StatusMessage);
		break;
	...
	case DeviceState::DeviceStateInError:
		...
		String strHRVal(hrVal);

		// Specifically handle a couple of known errors
		switch (hr)
		{
		case AUDCLNT_E_RESOURCES_INVALIDATED:
			message = String("ERROR: Endpoint Lost Access To Resources (") + strHRVal + ")";
			ShowStatusMessage(message, NotifyType::ErrorMessage);
			break;

		case E_ACCESSDENIED:
			message = String("ERROR: Access Denied (") + strHRVal + ").  Check 'Settings->Permissions' for access to Microphone.";
			ShowStatusMessage(message, NotifyType::ErrorMessage);
			break;

		default:
			message = String("ERROR: ") + strHRVal + " has occurred.";
			ShowStatusMessage(message, NotifyType::ErrorMessage);
		}
	}
...
void OculusWorldDemoApp::ShowStatusMessage(String message, NotifyType notifyType)
{
	Menu.SetPopupMessage(message);
}
